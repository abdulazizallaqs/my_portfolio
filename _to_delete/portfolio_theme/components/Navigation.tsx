      ...
      <div className="container-custom">
        <div className="flex items-center justify-between w-full">
          ...
          <div className="flex items-center">
            ...
            <div className="hidden md:flex items-center space-x-8 flex-nowrap">
              {navItems.map((item) => (
                <a
                  key={item.href}
                  href={item.href}
                  className="text-slate-300 hover:text-cyan-400 font-medium transition-all duration-300 relative group py-2 whitespace-nowrap"
                >
                  {item.label}
              ...
              <a
                href="#contact"
                className="ml-4 px-6 py-2.5 bg-gradient-to-r from-cyan-600 to-blue-600 text-white rounded-full font-medium hover:from-cyan-700 hover:to-blue-700 transition-all duration-300 flex items-center gap-2 shadow-lg hover:shadow-xl transform hover:scale-105"
              >
                <Mail size={16} />
                Contact Me
              </a>
            ...
            {/* Mobile Menu Button */}
            <button
              className="md:hidden p-2 rounded-full hover:bg-cyan-500/10 transition-all duration-300 backdrop-blur-sm"
        ...
        {/* Mobile Navigation */}
        {isOpen && (
          <div className="md:hidden mt-4 py-4 border-t border-cyan-500/20 bg-ink-900/80 backdrop-blur-md rounded-2xl">
            {navItems.map((item) => (
              <a
                key={item.href}
                href={item.href}
