    ...
    <SectionBackground variant="quaternary" className="section-padding" id="experience">
      <div className="container-custom px-4 sm:px-6">
        ...
        >
          ...
          <h2 className="relative z-10 text-4xl sm:text-6xl font-bold heading-gradient mb-6">
        ...
        <div className="max-w-6xl mx-auto" ref={timelineRef}>
          <div className="relative">
            ...
            <div className="absolute start-6 sm:start-1/2 sm:transform sm:-translate-x-1/2 rtl:sm:translate-x-1/2 top-0 bottom-0 w-1 bg-gradient-to-b from-cyan-200 via-blue-300 to-blue-200 rounded-full"></div>
            
            {/* Animated Progress Line with Glow */}
            <div 
              className={`absolute start-6 sm:start-1/2 sm:transform sm:-translate-x-1/2 rtl:sm:translate-x-1/2 top-0 w-1 bg-gradient-to-b from-cyan-500 via-blue-500 to-blue-500 rounded-full transition-all duration-3000 ease-out ${
            ...
            <div className="absolute start-6 sm:start-1/2 sm:transform sm:-translate-x-1/2 rtl:sm:translate-x-1/2 top-0 bottom-0 pointer-events-none">
              {[...Array(5)].map((_, i) => (
                <div
                  key={i}
...
function TimelineCard({ experience, index, inView, isLast, t }: TimelineCardProps) {
  ...
  return (
    ...
    >
      ...
      <div className="absolute start-6 sm:start-1/2 sm:transform sm:-translate-x-1/2 rtl:sm:translate-x-1/2 z-20">
        <div 
          className={`relative w-8 h-8 rounded-full border-4 border-ink-950 transition-all duration-500 cursor-pointer group ${
      ...
      <div className={`ms-20 sm:ms-0 ${
        isEven ? 'sm:pe-12 sm:text-end' : 'sm:ps-12'
      } sm:w-1/2 ${
        isEven ? 'sm:me-auto' : 'sm:ms-auto'
      }`}>
        <div 
        ...
        >
          ...
          <div className="relative z-10">
            ...
            <div className="mb-6">
              ...
              <h3 className="relative z-10 text-2xl sm:text-3xl font-bold heading-gradient mb-3 leading-tight">
                {experience.title}
              </h3>
              ...
              <div className="flex items-center gap-6 mb-6 text-sm text-muted">
                {experience.achievements && (
                  <div className="flex items-center gap-2">
                    ...
                    <span>{experience.achievements.length} {t.achievements}</span>
                  </div>
                )}
                {experience.technologies && (
                  <div className="flex items-center gap-2">
                    <Code size={16} className="text-blue-500" />
                    <span>{experience.technologies.length} {t.technologies}</span>
                  </div>
                )}
              </div>
            )}
            ...
            {(experience.achievements || experience.technologies) && (
              <div className={`transition-all duration-500 overflow-hidden ${
                ...
                {experience.achievements && experience.achievements.length > 0 && (
                  <div className="mb-6">
                    <h4 className="text-lg font-semibold text-fg mb-4 flex items-center gap-2">
                    ...
                    <div className="space-y-3">
                      {experience.achievements.map((achievement, achIndex) => (
                        <div key={achIndex} className="flex items-start gap-3 group">
                          ...
                          <span className="text-body text-sm leading-relaxed group-hover:text-fg transition-colors duration-300">{achievement}</span>
                        </div>
                      ))}
                    </div>
                  </div>
